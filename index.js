...
const boardWidth = 9; // 30
const boardHeight = 9; // 16
...
const flag = idx => {
	...
	const element = boardElements[idx];
	boardFlagged[idx] = (boardFlagged[idx] + 1) % 3;
	switch (boardFlagged[idx]) {
		case 0:
...
const main = () => {
	...
	while (!hasNine) {
		...
		generateBoard();
		for (let i = 0; i < boardWidth * boardHeight; i++) {
			hasNine = countNeighboringMines(i) == 9;
			if (hasNine) break;
		}
	}
